...
from pyrit.output.scenario_result.base import (
    ScenarioAttacksPrinterBase,
    ScenarioResultPrinterBase,
    ScenarioTechniqueMetricsPrinterBase,
)
...
class PrettyScenarioAttacksPrinter(_PrettyPrinterMixin, ScenarioAttacksPrinterBase):
    """Human-readable renderer for a scenario's per-attack view."""
    ...
    def __init__(self, *, sink: Sink | None = None, enable_colors: bool = True) -> None:
        """
        Initialize the per-attack renderer.
    ...
    async def render_async(self, payload: AttacksTablePayload) -> str:
        """
        Render per-attack scenario results.
        ...
        lines = [
            self._format_colored(
                f"\n  Attack Results — scenario {payload.scenario_result_id}",
                Style.BRIGHT,
                Fore.CYAN,
            )
        ]
        for index, row in enumerate(payload.rows, start=1):
            color = self.OUTCOME_COLORS.get(row.outcome)
            colors = (Style.BRIGHT, color) if color else ()
            score = row.score_value if row.score_value is not None else "—"
            lines.append(
                self._format_colored(
                    f"  {index}. [{row.outcome.upper()}] turns={row.executed_turns}  score={score}",
                    *colors,
                )
            )
            lines.extend(
                [
                    f"       id:        {row.attack_result_id}\n",
                    f"       technique: {row.atomic_attack_name}\n",
...
class PrettyScenarioTechniqueMetricsPrinter(ScenarioTechniqueMetricsPrinterBase):
    """Human-readable renderer for scenario technique metrics."""

    async def render_async(self, payload: TechniqueMetricsPayload) -> str:
        """
        Render scenario technique metrics as a fixed-width table.
        ...
        lines.extend(
            ...
            ).format(**metric.model_dump())
            for metric in payload.root
        )
        return "\n".join(lines) + "\n"
